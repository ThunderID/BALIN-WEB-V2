<div class="navbar navbar-default navbar-fixed-bottom navbar_shortcut hidden-lg hidden-md hidden-sm col-xs-12 border-top-1 border-grey" role="navigation">
	<div class="nav navbar-nav text-center mt-0 mb-0">
		...
		<div onclick="location.href='{{ route('balin.product.index', ['categories[]' => 'wanita']) }}';" class="col-xs-3 text-center border-right-1 border-grey pt-xs pb-xs cursor-pointer">
			...
			<p class="text-sm mb-0 @if(isset(Input::get('categories')[0]) && Input::get('categories')[0] == 'wanita') text-orange @endif">Wanita</p>
		</div>
		<div onclick="location.href='{{ route('balin.product.index', ['categories[]' => 'pria']) }}';" class="col-xs-3 text-center border-right-1 border-grey pt-xs pb-xs cursor-pointer">
			...
			<p class="text-sm mb-0 @if(isset(Input::get('categories')[0]) && Input::get('categories')[0] == 'pria') text-orange @endif">Pria</p>
		</div>		
		@if (Session::has('whoami'))
